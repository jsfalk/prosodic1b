...
def separate_vowels(vowels, boundary_list, start):
    ...
    if v_len == 2 and not is_inseparable_vowels(vowels):          
        ...
        if vowels in vowel_seq_dict:
            ...
            boundary_list[start+1:start+v_len] = vowel_seq_dict[vowels][1:-1] # ignore initial syllable separator and first vowel
        else:
...
def make_syllables(word):    
    ...
    if entry in pre_sep_dict: # introduces annotations, but will still be syllabified so that only partial annotations are required
        ...
        for i in range(1, len(entry)):
            boundary_list += [0 if entry[i] != "'" else 1]
        boundary_list += [1]
...
# return true if word has at least two moras starting from index to the end
def potential_word_index_to_end(word, index):
	vowel_count = 0
...
def make_splits(word, boundary_list):
    ...
    v_seq_start = 0
    v_seq_end = 0    
    # number of vowels encountered to avoid splitting initial consonant clusters
    # also decides whether to split a cluster as if it were a compound boundary
    # (words must have at least two moras)
    vowel_count = 0    
    for i in range(len(word)):        
        if is_vowel(word[i]): # continuing or starting vowel sequence            
            vowel_count += 1
            v_seq_end += 1            
            # deals with initial consonant clusters
            if vowel_count == 1:
                continue
            ...
            if v_seq_end - v_seq_start == 1:                
                ...
                for onset_length in onset_lengths:
                    ...
                    if cluster_start >= 0 and consonantal_onset(word[cluster_start:i]):                        
                        # do not split as if a compound if the preceding word would have less than 2 moras
                        if onset_length > 1:
                            if vowel_count < 3: # an extra vowel has been added, so 2 moras plus 1
                                continue                        
                        ...
                        if no_syllable_break:
                            if onset_length > 1:
                                # do not split as if cluster if no word to the right
                                if not potential_word_index_to_end(word, i):
                                	continue
                                vowel_count = 1 # split as if cluster, starting new word having encountered one vowel
