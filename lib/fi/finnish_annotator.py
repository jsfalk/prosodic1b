...
from finnish_syllables import initialize_presyllabified, make_syllables, potential_word_index_to_end
...
dirself = os.path.join(os.getcwd(), 'lib', 'fi')
user_files = [os.path.join(dirself,'presyllabified.txt'), os.path.join(dirself,'initial.txt'), os.path.join(dirself,'suffix.txt'), os.path.join(dirself,'compound.txt'), os.path.join(dirself,'function_words.txt')] # default values, in case user input is ill-formed or unavailable
...
def initialize_list(l, filename):
    try:
        ...
        for entry in entries:
            l.append(unicode(entry.lower(), 'utf-8'))
    except IOError:        
...
def initialize_dict_from_file(dict, filename):    
    try:        
        ...
        separator = entries[0]
        entries = [unicode(entry, 'utf-8') for entry in entries[1:]]
...
def initialize_config():
    ...
    initialize_list(function_words, user_files[FUNCTION])

initialize_config()

def is_lexical(word):
    word = word.lower()
    if not potential_word_index_to_end(word, 0):
        return False
    function_words = []
    initialize_list(function_words, user_files[FUNCTION])
    return not word in function_words
...
# combine subminimal morphs (CVC*)
def attach_subminimal_morphs(morphs):
	# if first morph is subminimal, attach to the right
	if not potential_word_index_to_end(morphs[0], 0) and len(morphs) > 1:
		morphs[1] = morphs[0] + morphs[1]
		del morphs[0]
	# for all other morphs, attach to the left if subminimal
	i = 1
	while i < len(morphs):
		if not potential_word_index_to_end(morphs[i], 0):
			morphs[i-1] += morphs[i]
			del morphs[i]
		else:
			i += 1
...
def make_annotation(word):
    ...
    words = attach_subminimal_morphs(words)
...
# return the string annotation of a word (in IPA)
def ipa_annotation(word):
    return to_ipa(annotation_string(make_annotation(word)))[1:-1]
...
def annotation_string(word_annotation):
    ...
    for i in range(len(word_annotation.stresses)):
        ...
        result += '\n'
        break # TEMPORARY, RETURN ONLY ONE ANNOTATION
